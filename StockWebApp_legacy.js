...
function StockWebAppLegacy_doGet_() {
  const initialResult = StockWebApp_collectPayload_(INITIAL_SERVER_RENDER_COUNT);
  const template = HtmlService.createTemplateFromFile("StockMobile_legacy");
...
function StockWebApp_collectHistoryPayload_(input) {
  ...
  const limit = Math.max(1, Math.min(50, StockWebApp_toInt_(payload.limit) || 20));
